...
export default function App() {
 ...
 return <main className="app-shell"><header className="compact"><button className="back" onClick={() => setSelected(null)}>← Opérations</button><p className="eyebrow">{selected.name}</p><h1>TABLEAU DE BORD</h1></header>{error && <p className="notice error">{error}</p>}{tab === 'dashboard' && <Dashboard operation={selected} units={units} lots={lots}/>}{tab === 'visit' && <VisitForm operationId={selected.id} units={units} lots={lots}/>} {tab === 'admin' && <Administration operationId={selected.id} units={units} lots={lots} companies={companies} assignments={assignments} reload={loadData}/>}<nav><button className={tab === 'dashboard' ? 'active' : ''} onClick={() => setTab('dashboard')}>Tableau de bord</button><button className={tab === 'visit' ? 'active' : ''} onClick={() => setTab('visit')}>Visite</button><button className={tab === 'admin' ? 'active' : ''} onClick={() => setTab('admin')}>Administration</button></nav></main>
}
