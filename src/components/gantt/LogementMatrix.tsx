...
function cellColor(task: GanttTask | undefined, today: Date): { bg: string; fg: string; label: string } {
  if (!task) return { bg: '#f3f6f9', fg: '#b8c4ce', label: '—' }
  if (task.progress >= 100) return { bg: '#dcfce7', fg: '#15803d', label: '100%' }
  if (isLate(task, today) || task.status === 'delayed' || task.status === 'blocked') {
    return { bg: '#fdecec', fg: '#b91c1c', label: `${task.progress}%` }
  }
  if (task.progress > 0) return { bg: '#e7f0fb', fg: '#185fa5', label: `${task.progress}%` }
  return { bg: '#f3f6f9', fg: '#5c6f80', label: '0%' }
}
...
export default function LogementMatrix({ tasks }: LogementMatrixProps) {
  ...
  return (
    <div style={{ overflowX: 'auto', border: '1px solid #e3e9ee', borderRadius: '8px', background: '#fff' }}>
      <table style={{ borderCollapse: 'collapse', width: 'max-content', minWidth: '100%', fontSize: '12px' }}>
        <thead>
          <tr>
            <th style={{ position: 'sticky', left: 0, zIndex: 2, background: '#f8fafc', padding: '10px 12px', textAlign: 'left', borderBottom: '1px solid #e3e9ee', minWidth: '140px', color: '#0b3b60', fontSize: '11px' }}>
              Lot \ Logement
            </th>
            {LOGEMENTS.map(l => (
              <th key={l.id} style={{ padding: '8px 6px', textAlign: 'center', borderBottom: '1px solid #e3e9ee', borderLeft: '1px solid #eef2f6', color: '#0b3b60', fontSize: '10px', minWidth: '64px', whiteSpace: 'nowrap' }}>
                {l.label}
        ...
        <tbody>
          {lots.map(lot => {
            ...
            return (
              <tr key={lot.id}>
                <td style={{ position: 'sticky', left: 0, zIndex: 1, background: '#fff', padding: '8px 12px', borderBottom: '1px solid #eef2f6', fontWeight: 600, color: lot.is_critical ? '#b91c1c' : '#0b3b60', fontSize: '11px' }}>
